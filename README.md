...
1. Cleaning the data
First, I will load in the Python libraries I need and download the dataset off the World Happiness Report website https://worldhappiness.report/ed/2021/#appendices-and-data.
...
​
import warnings
warnings.filterwarnings("ignore") #No errors on print preview as long as the code isn't been run yet
...
['Country name',
 ...
 'Positive affect',
 'Negative affect']
Life Ladder or the Cantril Scale is a question where respondents are asked to rate their life on a scale from 0 through 10 where 0 is the worst possible life for you and 10 is the best possible life. This is the happiness score/index the models will try to predict.
...
['Country Name',
 ...
 'Generosity',
 'Perceptions Of Corruption']
The column names are now capitalized. I need to check the column data types.
...
Here we see that 7 columns use data type float64. If I want the models to run, I need to convert the column values to the type float32.
...
dtype: int64
335
All the predictors have missing 335 total missing values. Removing them would drastically change the models. Instead, I will replace those values with their respective column means instead.
...
dtype: int64
There are no more missing values. The last thing I need to do is write the data to a new csv file so I can graph it in Tableau.

happiness.to_csv("happiness_cleaned.csv") 
2. Modeling
2a. Linear Regression
First, I need to extract my predictors and response variable from the dataframe. Then, I will split the dataset into a training set and a test set. I will also create a validation set and for all 3, compute the Mean Squared Error (MSE). The MSE measures the average squared difference between estimated values and actual values. I want to find the model that best minimizes it.
...
models = pd.DataFrame(data = dict({'Y-Test' : y_test})) 
#Use the dict() function to add the data
​
...
For the Random Forest, Boosting, and Bagging Models we can graph the significance of our variables. I will write another function that allows me to convert the data to a .csv file so it can be graphed in Tableau.
...
def importance_models(name, model_reg):
    ...
    importance.to_csv("importance_models.csv") #write to .csv so the data can be graphed in tableau
    print(importance)
...
Variable%20Importance%20for%20Random%20Forest.png

The two most important variables are "Log GDP Per Capita" and "Healthy Life Expectancy".

add_predictions('RF Predictions', rf_reg.predict(X_test))
Random%20Forest.png

2d. Boosting Model
I will test the Boosting Model.
...
The Boosting Model's test mean squared error is .239 and its cross validation mean squared error is .223.
...
Variable%20Importance%20for%20Boosting.png
...
Variable%20Importance%20for%20Bagging.png
...
Now I can compare the mean squared errors across all the models.
...
╘══════════════════════╧═══════╧═════════════════╧═════════════════╧════════════╧═══════════╛
3. Conclusions
As we can see in the Testing Performance Table, the Random Forest Model is the best with the lowest Testing and Cross-validation Mean Squared Errors of .229 and .198, respectively.
...
As seen in the Variable Importance table and each model's bar plots, the most important happiness predictors are in order: Log GDP Per Capita, Healthy Life Expectancy At Birth, Social Support, Freedom To Make Choices, Generosity, and Perceptions Of Corruption.

The Random Forest Model put Perceptions of Corruption as slightly more important than Generosity, unlike the Boosting and Bagging Models. The Random Forest Model also had a much lower importance value for Log GDP Per Capita than the Boosting and Bagging Models. Those models had Log GDP Per Capita as being the most important variable by far. Despite those small differences, the models primarily agree on the most important variables.

Log GDP Per Capita being the most important social and economic factor makes sense because when people are economically secure they are generally happier. Conversely, they are not happy when they are financially struggling because that jeopardizes their whole life.

My analysis had some limitations that I would like to discuss. For example, I substituted the missing values with the predictor means. This added some bias to the data because I might have underestimated or overestimated the actual values. A better approach would have been to predict them using Random Forests. Another limitation was ignoring the other happiness metrics of Positive Affect and Negative Affect. Moreover, there are other machine learning algorithms I did not look at that might have been better than Random Forest such as Logistic Regression and K-Means Clustering.

There is more information to find out from this dataset. I did not focus on any specific country or region for my analysis but they would likely vary in which factors are most important in predicting happiness. For example, I hypothesize in countries ruled by dictators, Perceptions of Corruption would be more important in predicting happiness than in my analysis where it was the least significant factor. I also did not look at specific years. In 2020, Social Support could have been more important in predicting happiness due to governments giving people more money. These are worth delving into.
